...
from src.modules.employee.schemas.hourTarget import EmployeeHourTargetRead, EmployeeHourTargetCreate, EmployeeHourTargetUpdate
...
def get_targets(emp_id: str, db: Session = Depends(get_db)):
    return hour_contract_crud.get_by_employee(db, emp_id)
...
def create_target(emp_id: str, schema_in: EmployeeHourTargetCreate, db: Session = Depends(get_db)):
    return hour_contract_crud.create_with_employee(db, schema_in, emp_id)
...
def update_target(emp_id: str, target_id: int, schema_in: EmployeeHourTargetUpdate, db: Session = Depends(get_db)):
    db_obj = db.query(hour_contract_crud.EmployeeHourTarget).filter(
        hour_contract_crud.EmployeeHourTarget.id == target_id,
        hour_contract_crud.EmployeeHourTarget.employee_id == emp_id
    ).first()
    if not db_obj: raise HTTPException(404, "Target not found")
    return hour_contract_crud.update(db, db_obj, schema_in)
...
def delete_target(emp_id: str, target_id: int, db: Session = Depends(get_db)):
    ...
    if not db_obj: raise HTTPException(404, "Target not found")
    hour_contract_crud.delete(db, target_id)